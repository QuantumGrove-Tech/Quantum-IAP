...
class BillingService(
    ...
    override fun buy(activity: Activity, sku: String) {
        if (!sku.isProductReady()) {
            log("buy. billing is not ready yet. (SKU is -1)")
            return
        }
    ...
    override fun subscribe(activity: Activity, sku: String) {
        if (!sku.isProductReady()) {
            log("buy. billing is not ready yet. (SKU is -2)")
            return
        }
    ...
    fun getSubPriceByTag(sku: String): String {
        var price = ""
        productDetails[sku]?.subscriptionOfferDetails?.getOrNull(0)?.pricingPhases?.pricingPhaseList?.forEach {
